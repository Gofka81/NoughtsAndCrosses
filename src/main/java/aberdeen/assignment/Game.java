...
public class Game {
    ...
    public BoxStatus checkDiagonal(){
        if (!isEmpty(5) && (getBox(1) == getBox(5) && getBox(5) == getBox(9))
        || (getBox(3) == getBox(5) && getBox(5) == getBox(7)))
            return getBox(5);
        return BoxStatus.Empty;
    }
    ...
    public GameStatus getWinner(BoxStatus winnersSquare){
        if (winnersSquare == BoxStatus.Human)
    ...
    public GameStatus gameStatus(){
        if (checkHorizon() != BoxStatus.Empty)
            return getWinner(checkHorizon());
        if (checkVertical() != BoxStatus.Empty)
            return getWinner(checkVertical());
        if (checkDiagonal() != BoxStatus.Empty)
            return getWinner(checkDiagonal());
        if(isTie())
